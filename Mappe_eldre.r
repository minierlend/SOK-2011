...
labs <- c("Gjennomsnitlig årlig vekstrate i BNP pc 2000-2019 (%)", "Gjennomsnittlig årlig befolkningsvekst (%)",
          ...
          "Vekstrate i eksport", "Antall år i skole", "BNP per innbygger(LOG)") 
...
plot1 <- ggplot(no_outliers, aes(x = avg_nsy , y = ln_gdppc, na.rm = TRUE)) +
  ...
  geom_point(aes(size = poptot, color = region), alpha = 0.5) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse # Bestem font-størrelse på legend
...
plot3 <- ggplot(no_outliers, aes(x = avg_educ , y = ln_gdppc, na.rm = TRUE)) +
  xlab("Gjennomsnittlig antall år i utdanning") + 
  ylab("BNP per innbygger 2019(log)") + 
  geom_point(aes(size = poptot, color = region), alpha = 0.5) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse # Bestem font-størrelse på legend
  scale_colour_manual(values = rainbow(9)) +# Velg farger til bobblene
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_rect(fill = 'white'))+
  labs(title = "Forholdet mellom utdanningsnivå og BNP per innbygger") +
  geom_smooth(method="lm", se = FALSE)+
  theme_grey()# logaritmere BNP pc og velg hvilke "ticks" som skal vises
plot3
...
plot5 <- ggplot(no_outliers, aes(x = avg_educ , y = avg_gdpgrowth, na.rm = TRUE)) +
  xlab("Gjennomsnittlig utdanningsnivå, humankapital ") + 
  ylab("Vekstraten i BNP per innbygger") + 
  geom_point(aes(size = poptot, color = region), alpha = 0.5) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse # Bestem font-størrelse på legend
  scale_colour_manual(values = rainbow(9)) +# Velg farger til bobblene
